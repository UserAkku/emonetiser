...
export default function CtvMonetizationPage() {
  return (
    <main className="flex flex-col w-full bg-background min-h-screen text-foreground">
      ...
      <PageSection className="pt-32 pb-24 md:pt-40 md:pb-32 gradient-mesh">
        <Container>
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-12 lg:gap-16 items-center">
            ...
            <FadeUp delay={0.2} className="relative w-full aspect-video rounded-2xl bg-card/50 backdrop-blur-xl border border-border shadow-2xl overflow-hidden flex flex-col justify-between group">
              ...
              <div className="flex justify-between items-start p-6 relative z-10">
                <div className="bg-background/60 backdrop-blur-md border border-border text-foreground text-[10px] font-bold px-3 py-1.5 rounded-full uppercase tracking-widest flex items-center gap-2 shadow-sm transition-colors group-hover:border-border">
              ...
              <div className="absolute inset-0 flex items-center justify-center pointer-events-none z-10">
                <div className="w-16 h-16 rounded-full bg-background/40 backdrop-blur-md border border-border flex items-center justify-center transition-transform duration-500 group-hover:scale-110 group-hover:border-[var(--color-brand-primary)]/30 group-hover:bg-[var(--color-brand-primary)]/10 shadow-xl">
              ...
              <div className="flex flex-col gap-5 mt-auto relative z-10 p-6 bg-gradient-to-t from-background/90 via-background/40 to-transparent">
                ...
                {/* Scrubber Bar */}
                <div className="w-full h-1 bg-black/5 rounded-full relative overflow-visible flex items-center group-hover:h-1.5 transition-all duration-300">
          ...
          <StaggerContainer className="grid grid-cols-1 md:grid-cols-2 gap-6">
            <GlassPanel className="p-8 md:col-span-2 bg-gradient-to-r from-black/5 to-transparent flex flex-col justify-end min-h-[320px]">
              ...
              <div className="flex flex-col md:flex-row md:items-end gap-4 justify-between">
                <div>
                  <h3 className="text-2xl font-medium text-foreground mb-2">Premium Attention</h3>
                  <p className="text-muted-foreground">Captive audiences leading to higher engagement.</p>
                </div>
                <div className="text-4xl md:text-6xl font-mono text-foreground">85% <span className="text-sm md:text-base text-slate-500 font-sans tracking-widest uppercase block md:inline">Completion Rate</span></div>
              </div>
            </GlassPanel>
            ...
            <GlassPanel className="p-8 md:col-span-2 flex flex-col md:flex-row items-center gap-8 bg-gradient-to-bl from-[var(--color-brand-primary)]/5 to-transparent">
              <div className="flex-1">
                ...
                <p className="text-muted-foreground text-sm">CTV inventory commands premium pricing due to scarcity and high impact.</p>
              </div>
              <div className="bg-black/20 p-6 rounded-xl border border-border">
                <div className="text-4xl md:text-5xl font-mono text-foreground font-semibold">3-5×</div>
      ...
      {/* S3 SSAI EXPLANATION */}
      <PageSection className="py-24 md:py-32 overflow-hidden border-y border-border">
        <Container>
          <FadeUp className="mb-16">
            <h2 className="text-3xl md:text-5xl font-display text-foreground text-center">Seamless Server-Side Delivery</h2>
          </FadeUp>
          
          <div className="relative w-full max-w-5xl mx-auto hidden md:block">
            {/* Connecting Line */}
            <div className="absolute top-[40%] left-0 w-full h-[2px] bg-gray-200 z-0" />
              ...
              {[
                ...
                <div key={i} className="flex flex-col items-center text-center">
                  <div className={cn(
                    ...
                    step.highlight ? "bg-[var(--color-brand-primary)]/20 border border-[var(--color-brand-primary)] shadow-[0_0_30px_rgba(227,93,46,0.3)] glow-primary" : "bg-white/[0.04] border border-border"
                  )}>
                    <div className={cn("text-lg font-bold", step.highlight ? "text-[var(--color-brand-primary)]" : "text-foreground")}>{i + 1}</div>
                  </div>
                  <h4 className={cn("font-medium mb-2", step.highlight ? "text-[var(--color-brand-primary)]" : "text-foreground")}>{step.title}</h4>
          ...
          <div className="md:hidden space-y-6">
             {[
                ...
                <div key={i} className={cn("p-4 rounded-xl border flex gap-4 items-center", step.highlight ? "border-[var(--color-brand-primary)] bg-[var(--color-brand-primary)]/10" : "border-border bg-black/5")}>
                   <div className="w-10 h-10 shrink-0 rounded-lg bg-black/30 flex items-center justify-center font-bold text-foreground">{i + 1}</div>
                   <div>
      ...
      <PageSection className="py-24 md:py-32 bg-gray-50">
        <Container>
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-16">
            <FadeUp>
              <h3 className="text-2xl font-display text-foreground mb-8">Supported Platforms</h3>
              <div className="flex flex-wrap gap-4">
                {['Roku', 'Apple TV', 'Fire TV', 'Android TV', 'Samsung Tizen', 'LG webOS', 'Web OTT'].map((platform, i) => (
                  <div key={i} className="flex items-center gap-2 px-4 py-3 bg-black/5 border border-border rounded-full text-muted-foreground font-medium">
                    <Television className="w-5 h-5 text-muted-foreground" />
                    {platform}
                  </div>
                ))}
              </div>
            ...
            <StaggerContainer>
              <h3 className="text-2xl font-display text-foreground mb-8">Video Formats</h3>
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                {['Pre-Roll', 'Mid-Roll', 'Post-Roll', 'Companion Ads', 'Overlay/Squeeze'].map((format, i) => (
                  <div key={i} className="p-4 bg-gradient-to-br from-black/5 to-transparent border border-border rounded-xl">
                    <div className="text-foreground font-medium">{format}</div>
                  </div>
                ))}
              </div>
            </StaggerContainer>
        ...
        <Container>
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-16 items-center">
            <FadeUp>
              ...
              <h2 className="text-3xl md:text-5xl font-display text-foreground mb-6">Keep your streams pristine.</h2>
            ...
            <StaggerContainer className="space-y-4">
              ...
              <GlassPanel className="p-6 flex items-start gap-4">
                ...
                <div>
                  ...
                  <p className="text-sm text-muted-foreground">Full support for Open Measurement and VAST 4.0 specs.</p>
                </div>
              </GlassPanel>
            </StaggerContainer>
          </div>
